									...
									<table>
										...
										@foreach($pendientes as $indice=>$pndt)
											<tr>
												...
												@if ($solicitud_a > 0)
													<td>{{ $pndt->cnombre_personal.' '.$pndt->cpaterno_personal.' '.$pndt->cmaterno_personal.',' }}<br>
														{{ substr($pndt->cdescripcion_puesto,0,44) }}
												...
												@else
													<td>{{ $pndt->personalremitente->cnombre_personal.' '.$pndt->personalremitente->cpaterno_personal.' '.$pndt->personalremitente->cmaterno_personal.',' }}<br>
														{{ substr($pndt->cdescripcion_puesto,0,44) }}
														@if(strlen($pndt->cdescripcion_puesto)>44)
															<br>{{ substr($pndt->cdescripcion_puesto,44,44) }}
														@endif
